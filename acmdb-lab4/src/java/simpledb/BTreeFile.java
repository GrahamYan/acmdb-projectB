...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		while (pid.pgcateg() == BTreePageId.INTERNAL) {
			...
			for (Iterator<BTreeEntry> tmp = page.iterator(); tmp.hasNext() && next == null; ) {
				BTreeEntry entry = tmp.next();
		...
		assert pid.pgcateg() == BTreePageId.LEAF;
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage leaf = (BTreeLeafPage)getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		Iterator<Tuple>iter = page.iterator();
		if (iter == null || !iter.hasNext()) {
			throw new DbException("BTreeFile_splitLeafPage_iter=null");
		}
		int num = page.getNumTuples();
		for (int i = 1; i + i <= num; i++) {
			if (!iter.hasNext()) throw new DbException("BTreeFile_splitLeafPage_hasNext=false");
			Tuple tmp = iter.next();
			page.deleteTuple(tmp);
			leaf.insertTuple(tmp);
		}
		if (page.getLeftSiblingId() != null) {
			BTreeLeafPage tmp = (BTreeLeafPage)getPage(tid, dirtypages, page.getLeftSiblingId(),Permissions.READ_ONLY);
			tmp.setRightSiblingId(leaf.getId());
		}
		leaf.setLeftSiblingId(page.getLeftSiblingId());
		page.setLeftSiblingId(leaf.getId());
		leaf.setRightSiblingId(page.getId());
		Field f = iter.next().getField(keyField);
		BTreeEntry entry = new BTreeEntry(f, leaf.getId(), page.getId());
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeInternalPage page, Field field) 
					throws DbException, IOException, TransactionAbortedException {
		// some code goes here
		BTreeInternalPage internal = (BTreeInternalPage)getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		Iterator<BTreeEntry> iter = page.iterator();
		int num = page.getNumEntries();
		if (iter == null || !iter.hasNext()) {
			throw new DbException("BTreeFile_splitInternalPage_iter=null");
		}
		for (int i = 1; i + i <= num; i++) {
			if (!iter.hasNext()) throw new DbException("BTreeFile_splitInternalPage_hasNext=false");
			BTreeEntry tmp = iter.next();
			page.deleteKeyAndLeftChild(tmp);
			internal.insertEntry(tmp);
		}
		BTreeEntry itToPush = iter.next();
		Field fieldToPush = itToPush.getKey();
		page.deleteKeyAndLeftChild(itToPush);
		itToPush = new BTreeEntry(fieldToPush, internal.getId(), page.getId());
		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, internal);
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), fieldToPush);
		parent.insertEntry(itToPush);
		updateParentPointers(tid, dirtypages, parent);
		if (fieldToPush.compare(Op.GREATER_THAN_OR_EQ, field)) {
			return internal;
		}
		else {
			return page;
